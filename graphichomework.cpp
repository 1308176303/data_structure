...
int main(int argc, char *argv[]) {
    ...
    if (g.isInDriected()) cout << "无向图\n";
    else cout << "有向图\n";

    if (g.isDAG()) cout << "有向无环图\n";
    else cout << "不是有向无环图\n";

    if (g.isConnect()) cout << "连通图\n";
    else cout << "不是连通图\n";
        ...
        for(int i=1;i<=nsum;i++)
        {
            for(int j=1;j<=nsum;j++)
            {
                if(matrix[i][j]==MAXINT)
                    cout<<"INF   ";
                else cout<<left<<setw(5)<<matrix[i][j]<<" ";//对齐多少宽度根据边权重的量级修改
            }
            cout<<endl;
        }
    }
...
void graph::dijkstra(int source)
{
    ...
    {
        if (dist[i] == MAXINT)
            cout << "从 " << source << " 到 " << i << " 的路径不存在" << endl;
        else
            cout << "从 " << source << " 到 " << i << " 的最短路径为: " << dist[i] << endl;
    }
}
